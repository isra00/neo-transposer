	...
	<div class="teaser">
		<div class="inside">
			<div class="more-inside">
				<p>{% trans with {'%song%': song.song_details.title } %}Neo-Transposer helps you to automatically transpose the chords of <strong>%song%</strong> so they match your voice. Type your e-mail, follow the steps and it will transpose all the songs of the Neocatechumenal Way for you!{% endtrans %}</p>
...
<h1>
	...
	<small>{% if app.debug %}[{{ song.song_details.id_song }}]{% endif %}</small>
</h1>
...
{% for transposition in song.transpositions %}
	{{ self.printTransposition(transposition, song.original_chords) }}
{% endfor %}
</div>
...
<div class="transpositions-list">
	{{ self.printTransposition(not_equivalent, song.original_chords) }}
</div>
{% endif %}
...
NT = {
	...
	showChart: function(oLinkContainer)
	{
		...
		ga('send', 'event', 'Actions', 'ShowVoiceChart', '{{ song.song_details.title }}');
	},
	...
	sendFeedback: function(iAnswer)
	{
		$.post(
			...
			{
				id_song: {{ song.song_details.id_song }},
				worked: iAnswer,
...
$(function() {
	...
	$("#feedback-yes").click(function(e) {
		...
		NT.sendFeedback(1);
		ga('send', 'event', 'FeedbackTransposition', 'Worked', '{{ song.song_details.title }}');
	});
	...
	$("#feedback-no").click(function(e) {
		...
		$("#reasons-no").show();
		NT.sendFeedback(0);
		ga('send', 'event', 'FeedbackTransposition', 'NotWorked', '{{ song.song_details.title }}');
	});
