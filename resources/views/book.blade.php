...
@section('header_extra')
    ...
    @foreach ($all_books as $id => $book)
        <link rel="alternate" hreflang="{{ $book->locale() }}" href="{{ url(route('book_' . $id)) }}" />
    @endforeach
@endsection
...
@section('content')
    ...
    @if (config('nt.show_manifesto') && app()->getLocale() === 'es' && !$show_unhappy_warning && !$show_encourage_fb)
        <a href="{{ route('manifesto', ['locale' => app()->getLocale()]) }}">
            <div class="tip manifesto">
                <span class="boss">Manifiesto</span><br>
                Neo-Transposer de desarrollo web
            </div>
        </a>
    @endif
...
<div class="teaser">
    <div class="inside">
        <div class="more-inside">
            <p>@lang('Log-in now to transpose automatically the chords of the songs of the Neocatechumenal Way.')</p>
            @include('partial_login_form', ['redirect' => Request::url()])
        </div>
    </div>
</div>
@endif
...
<span class="link-all-report-wrapper">
    <a class="link-all-report btn-neutral btn-icon icon-download" href="{{ route('all_songs_report', ['locale' => app()->getLocale()]) }}">@lang('Download or print <small>All songs report</small>')</a>
</span>
@endif
...
<ul class="song-index">
    ...
    <li>
        <a href="{{ route('transpose_song', ["id_song" => $song->slug]) }}">
            <span class="page-number">{!! $song->page ?? '&#248;'  !!} · </span>
            <h4>{{ $song->title }}</h4>
            @if(isset($song->worked) && $song->worked == '1')
            <span class="green" title="@lang('You have reported the proposed transposition as valid')">&#10004;</span>
            @endif
        </a>
    </li>
    @endforeach
...
@if (session('user')->isLoggedIn() && app()->getLocale() === 'es')
<aside class="tip">
...
<script>
    ...
    /** @see https://stackoverflow.com/a/42345059 */
