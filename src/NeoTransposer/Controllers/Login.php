...
final class Login
{
    ...
    public function get(Request $req, NeoApp $app, array $tpl_vars = []): string
    {
        ...
        $tpl_vars['languages']        = config('nt.languages');
        $tpl_vars['page_title']       = __('Transpose the songs of the Neocatechumenal Way · Neo-Transposer');
        $tpl_vars['meta_description'] = __('Transpose the songs of the Neocatechumenal Way automatically with Neo-Transposer. The exact chords for your own voice!');
        $tpl_vars['meta_canonical']   = $app['absoluteUriWithoutQuery'];

        return response()->view('login', $tpl_vars);
    }
    ...
    public function post(Request $req, NeoApp $app): string|\Symfony\Component\HttpFoundation\RedirectResponse
    {
        ...
        if (!$user = $userRepository->readFromEmail($req_email)) {
            ...
            $idBook = $bookRepository->readIdBookFromLocale(App::getLocale());
        ...
        if ($user->firstTime) {
            return redirect()->route('user_voice', ['_locale' => App::getLocale(), 'firstTime' => '1']);
        }
    ...
    {
        return response()->view('external_login_finish.twig');
    }
}
