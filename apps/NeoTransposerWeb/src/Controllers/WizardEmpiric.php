...
class WizardEmpiric
{
	...
	public function lowest(Request $req, NeoApp $app)
	{
		...
		if ('yes' == $req->get('can_sing'))
		{
			$app['neouser']->range = new NotesRange(
                $this->nc->transposeNote($app['neouser']->range->lowest(), -1),
                $this->nc->transposeNote($app['neouser']->range->highest(), -1)
            );
	...
	public function highest(Request $req, NeoApp $app)
	{
		...
		if ('yes' == $req->get('can_sing'))
		{
			...
			$app['neouser']->range = new NotesRange($app['neouser']->range->lowest(), $nc->transposeNote($app['neouser']->range->highest(), +1));
		...
		{
			$app['neouser']->range = new NotesRange($app['neouser']->range->lowest(), $this->nc->transposeNote($app['neouser']->range->highest(), -1));
	...
	public function prepareSongForTest($wizard_config_song, $forceVoiceLimit, NeoApp $app): array
	{
		...
		return [
			...
			'show_audio'	=> $app['neoconfig']['audio'] && file_exists($app['root_dir'] . '/public' . $audioFile),
	...
	public function finish(Request $req, NeoApp $app): RedirectResponse
    {
		...
		$app[UnhappinessManager::class]->changedVoiceRangeFromWizard($app['neouser']);
