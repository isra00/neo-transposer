...
class ServeCss
{
	 ...
	 *
	 * @param \NeoTransposerWeb\NeoApp $app The Silex app.
	 */
	public function get(\NeoTransposerWeb\NeoApp $app): \Symfony\Component\HttpFoundation\RedirectResponse
	{
		$minified_css = $this->minify_css(file_get_contents($app['root_dir'] . '/public' . $this->src_file));
		$minified_hash 	= md5($minified_css);

		file_put_contents(
			$app['root_dir'] . '/public' . sprintf($this->min_file, $minified_hash),
			$minified_css
		);
